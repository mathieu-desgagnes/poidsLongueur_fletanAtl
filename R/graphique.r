...
# à faire: débugger en incluant les données nécessaires à PBSmapping
## Figure 25: carte de la distribution des poissons considérés dans relation poids-longueur
## mlFit$donnees@ données utilisées pour calculer la relation masse-longueur
carteMasseLong <- function(mlFit, langue='fr', xlim1=c(-70, -55), ylim1=c(45.5, 52.4)){
    ...
    addLines(ssZone$opano$lignes.zone, , lwd=1, lty=1, col=2)
        ...
        ## addPoints(as.EventData(cbind(EID=1:nrow(temp), temp), projection='LL'), col='grey70', cex=0.5)
    points(mlFit$donnees[mlFit$donnees$source=='ngsl',c('X','Y')], pch=16, cex=0.5, col=2)
    points(mlFit$donnees[mlFit$donnees$source=='sgsl',c('X','Y')], pch=16, cex=0.5, col=3)
    legend('topleft', inset=0.03, legend=paste(c('ngsl,','sgsl,'),'n =', table(mlFit$donnees$source)), bg='white', pch=16, col=c(2,3))
...
relationMasseLongueur <- function(mlFit, langue='fr', xlim=NULL, ylim=NULL, legende=TRUE, pct90=FALSE){
  ...
  #points(mlFit$donnees[mlFit$donnees$source=='ngsl',c('longueur','poidsKg')], pch=21, bg=2)
  #points(mlFit$donnees[mlFit$donnees$source=='sgsl',c('longueur','poidsKg')], pch=21, bg=3)
  temp_long <- seq(par('usr')[1], par('usr')[2], by=0.1)
    lines(temp_long, mlFit$l2p(temp_long), col=1, lwd=2)
        ...
        ## icGros <- quantile((exp(mlFit$donnees[,'logPkg'])/exp(mlFit$fitted.values))[mlFit$donnees$longueur>=84.99], probs=c(0,0.05,0.1,0.9,0.95,1))
    ...
    legend('topleft', inset=0.03,
           legend=paste0(c('NGSL, n=','SGSL, n='),c(sum(mlFit$donnees$source=='ngsl'),sum(mlFit$donnees$source=='sgsl'))), lty=c(NA,NA), pch=c(21,21),
           pt.bg=c(2,3), lwd=c(NA,NA))
}

residusML <- function(mlFit, langue='fr', ylim=NULL){
    switch(langue,
    ...
    ##
    plot(mlFit$donnees[,'longueur'], mlFit$donnees[,'residuels'],
         xlab=xlab, ylab=ylab, cex=0.7, ylim=ylim)
    #
    set.seed(1)
    quel_ordre <- sample(1:nrow(mlFit$donnees),replace=FALSE)
    points(mlFit$donnees[quel_ordre,c('longueur','residuels')], pch=21, bg=ifelse(mlFit$donnees[quel_ordre,'source']=='ngsl',2,3), cex=0.7)
    #points(mlFit$donnees[mlFit$donnees$source=='ngsl','longueur'], mlFit$donnees[mlFit$donnees$source=='ngsl','residuels'], pch=21, bg=2, cex=0.7)
    #points(mlFit$donnees[mlFit$donnees$source=='sgsl','longueur'], mlFit$donnees[mlFit$donnees$source=='sgsl','residuels'], pch=21, bg=3, cex=0.7)
    abline(h=0)
    ## lines(smooth.spline(mlFit$donnees[mlFit$donnees$source=='ngsl',c('longueur','residuels')], df=5), col=2, lwd=3)
    ## lines(smooth.spline(mlFit$donnees[mlFit$donnees$source=='sgsl',c('longueur','residuels')], df=5), col=3, lwd=3)
...
residusML.prof <- function(mlFit, probGraph=c(0.025,0.975), langue='fr', profCotier=-1, boxplot=FALSE){
    switch(langue,
    ...
    if(boxplot){
        ...
        boxplot(fulton~profMoy.classe, data=subset(mlFit$donnees, sexe=='5')); abline(h=0.005, col=4)
        boxplot(longueur~profMoy.classe, data=mlFit$donnees, ylim=c(0,150))
        boxplot(longueur~profMoy.classe, data=subset(mlFit$donnees, sexe=='1'), ylim=c(0,150))
        boxplot(longueur~profMoy.classe, data=subset(mlFit$donnees, sexe=='5'), ylim=c(0,150))
        boxplot(profMoy~longueur.classe, data=mlFit$donnees, ylim=c(0,500))
        boxplot(profMoy~longueur.classe, data=subset(mlFit$donnees, sexe=='5'), ylim=c(0,500), border='red')
        boxplot(profMoy~longueur.classe, data=subset(mlFit$donnees, sexe=='1'), ylim=c(0,500), add=TRUE)
        require('vioplot')
        vioplot(formula=fulton~profMoy.classe, data=mlFit$donnees); abline(h=0.001, col=4)
        vioplot(formula=residuels~profMoy.classe, data=mlFit$donnees, ylim=c(0.75, 1.5)); abline(h=1, col=4)
        vioplot(formula=residuels~longueur.classe, data=mlFit$donnees, ylim=c(0.75, 1.5)); abline(h=1, col=4)
    ...
    ## temp <- hist((mlFit$fitted.values/exp(mlFit$donnees[,'logPkg'])), breaks=c(0.3,1-sd.temp,1,1+sd.temp,1.85), plot=FALSE)
    abline(h=1)
    ...
    ## icPetit <- quantile((exp(mlFit$donnees[,'logPkg'])/mlFit$fitted.values)[mlFit$donnees$profMoy<=50.01], probs=probGraph)
    ...
    residusML.date <- function(mlFit, probGraph=c(0.025,0,975), langueFR=TRUE, ecran=FALSE, profCotier=-1, boxplot=FALSE){
        ...
        mlFit$donnees$residuels <- exp(mlFit$donnees[,'logPkg'])/exp(mlFit$fitted.values)
        mlFit$donnees$profMoy.classe <- as.numeric(cut(mlFit$donnees$profMoy, breaks=seq(0,500,by=25)))
        mlFit$donnees$longueur.classe <- as.numeric(cut(mlFit$donnees$longueur, breaks=seq(0,200,by=10)))
        mlFit$donnees$fulton <- 100*mlFit$donnees$poidsKg/(mlFit$donnees$longueur^3)
        if(boxplot){
            ...
            plot(mlFit$donnees[,'jourAnnee'], (exp(mlFit$donnees[,'logPkg'])/exp(mlFit$fitted.values)),
                 xlab=xlab, ylab=ylab, ylim=c(1/2,2), xlim=c(200,280), xaxs='i')
            points(mlFit$donnees[mlFit$donnees$source=='ngsl','jourAnnee'], (exp(mlFit$donnees[,'logPkg'])/exp(mlFit$fitted.values))[mlFit$donnees$source=='ngsl'], pch=21, bg=2)
            points(mlFit$donnees[mlFit$donnees$source=='sgsl','jourAnnee'], (exp(mlFit$donnees[,'logPkg'])/exp(mlFit$fitted.values))[mlFit$donnees$source=='sgsl'], pch=21, bg=3)
        }
        ## sd.temp <- sd((exp(mlFit$fitted.values)/exp(mlFit$donnees[,'logPkg'])))
        ## temp <- hist((exp(mlFit$fitted.values)/exp(mlFit$donnees[,'logPkg'])), breaks=c(0.3,1-sd.temp,1,1+sd.temp,1.85), plot=FALSE)
        abline(h=1)
        axis(4, at=c(1), labels=F, col.ticks=1)
        ##
        ## plus de 248 jours
        ic <- quantile((exp(mlFit$donnees[,'logPkg'])/exp(mlFit$fitted.values)), probs=probGraph)
        icGros <- quantile((exp(mlFit$donnees[,'logPkg'])/exp(mlFit$fitted.values))[mlFit$donnees$jourAnnee>=248.5], probs=probGraph)
        ...
        ## moins de 248jours
        ic <- quantile((exp(mlFit$donnees[,'logPkg'])/exp(mlFit$fitted.values)), probs=probGraph)
        icPetit <- quantile((exp(mlFit$donnees[,'logPkg'])/exp(mlFit$fitted.values))[mlFit$donnees$jourAnnee<248.5], probs=probGraph)
        lines(c(0,248.5), rep(icPetit[1],2), col=2)
        lines(c(0,248.5), rep(icPetit[2],2), col=2)
        ## moins de 50m, sum(mlFit$donnees$profMoy<=50.01)/nrow(mlFit$donnees)
        ## icPetit <- quantile((exp(mlFit$donnees[,'logPkg'])/exp(mlFit$fitted.values))[mlFit$donnees$profMoy<=50.01], probs=probGraph)
        ## lines(c(0,50), rep(icPetit[1],2), col=4)
        ## lines(c(0,50), rep(icPetit[2],2), col=4)
        ## text(par('usr')[1], icPetit[2], labels=names(icPetit[2]), adj=c(0,-0.2), col=2, cex=0.7)
        ## text(par('usr')[1], icPetit[1], labels=names(icPetit[1]), adj=c(0,1.1), col=2)
